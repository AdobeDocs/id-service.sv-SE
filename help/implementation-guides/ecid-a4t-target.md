...
* Synkronisera separata [!DNL Target]- och [!DNL Analytics]-ID:n för att avgöra om en besökare som ses av en lösning är samma person som den andra lösningen.
...
>[!NOTE]
>
>Den här metoden begär en enda mbox från [!DNL Target]. Om du behöver begära flera mbox i ett enskilt anrop, se [generateBatchPayload](https://www.npmjs.com/package/@adobe-mcid/visitor-js-server#generatebatchpayload).

Nyttolastbegäran ska se ut så här: I kodexemplet är funktionen `visitor.setCustomerIDs` valfri. Mer information finns i [Kund-ID:n och autentiseringstillstånd](../reference/authenticated-state.md).
...
När servern har tagit emot nyttolastdata från ID-tjänsten måste du initiera ytterligare kod för att sammanfoga den med data som skickats till [!DNL Target]. Det sista JSON-objektet som skickas till [!DNL Target] ser ut ungefär så här:
...
* AppMeasurementet skickar data om sidträffen till [!DNL Analytics], inklusive SDID.
* [!DNL Analytics] och [!DNL Target] jämför SDID:n för den här besökaren. Med ett identiskt SDID sammanfogar [!DNL Target] och [!DNL Analytics] anropet på serversidan och anropet på klientsidan. Nu ser båda lösningarna besökaren som samma person.

>[!MORELIKETHIS]
>
>* [Tjänstpaket för serversid-ID från nodpaketshanteraren](https://www.npmjs.com/package/@adobe-mcid/visitor-js-server)
