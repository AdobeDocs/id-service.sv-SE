---
description: Information om andra funktioner och funktioner i Experience Cloud Identity Service.
keywords: ID-tjänst
title: 'Referens '
exl-id: 9a14300c-206c-4960-b594-517809f7e979
source-git-commit: 06e935a4ba4776baa900d3dc91e294c92b873c0f
workflow-type: tm+mt
source-wordcount: '93'
ht-degree: 7%

---

# Referens  {#reference}

Information om andra funktioner och funktioner i Experience Cloud Identity Service.

* [Analysreferensöversikt](analytics-reference/analytics-reference.md)
* [Skyddsprofiler för innehåll och Experience Cloud Identity Service](csp.md)
* [Stöd för COPPA i Experience Cloud Identity Service](coppa.md)
* [CORS-stöd i Experience Cloud Identity Service](cors.md)
* [Kund-ID:n och autentiseringstillstånd](authenticated-state.md)
* [Hämta region- och användar-ID från AMCV Cookie eller ID-tjänsten](regions.md)
* [Krav för Experience Cloud Identity Service](requirements.md)
* [Heartbeat och Experience Cloud Identity Service](heartbeat.md)
* [Data Workbench och Experience Cloud Identity Service](dwb.md)
